...
static const char *FNAME = "tm_distortion.c";
...
int alloc_distortion_temp_buffers()
{
	...
	if (!(tmp_block = allocate_block()))  RETURN_ERR_CODE(ERR_ALLOC_FAIL);
	tmp = &tmp_block->data[0];
	if (!(low_block = allocate_block()))  RETURN_ERR_CODE(ERR_ALLOC_FAIL);
	low = &low_block->data[0];
	if (!(mid_block = allocate_block()))  RETURN_ERR_CODE(ERR_ALLOC_FAIL);
	mid = &mid_block->data[0];
	if (!(high_block = allocate_block())) RETURN_ERR_CODE(ERR_ALLOC_FAIL);
	high = &high_block->data[0];

	if (!(mid_distorted_block = allocate_block())) RETURN_ERR_CODE(ERR_ALLOC_FAIL);
	mid_distorted  = &mid_distorted_block->data[0];
	if (!(high_distorted_block = allocate_block())) RETURN_ERR_CODE(ERR_ALLOC_FAIL);
...
int init_distortion_str(tm_distortion_str *str)
{
	...
	init_setting_simple(&str->type, DISTORTION_TANH);
