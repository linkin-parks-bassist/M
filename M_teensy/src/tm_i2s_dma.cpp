...
tm_audio_block_int *i2s_in_block_left  = NULL;
tm_audio_block_int *i2s_in_block_right = NULL;

uint16_t	i2s_in_block_offset = 0;
bool 	 	i2s_in_update_responsibility = false;
DMAChannel 	i2s_in_dma(false);

tm_audio_block_int *i2s_out_block_left_1st  = NULL;
tm_audio_block_int *i2s_out_block_right_1st = NULL;
tm_audio_block_int *i2s_out_block_left_2nd  = NULL;
tm_audio_block_int *i2s_out_block_right_2nd = NULL;
...
extern "C" void init_i2s()
{
	...
	tm_printf("Configuring I2S... ");
	configure_i2s();
	tm_printf("done.\n");

	tm_printf("Configuring I2S input DMA channel...\n");
	CORE_PIN8_CONFIG  = 3;  //1:RX_DATA0
	IOMUXC_SAI1_RX_DATA0_SELECT_INPUT = 2;

	tm_printf("Setting to DMA registers... ");
	...
	tm_printf("Registering update interrupt service routine... ");
	i2s_in_update_responsibility = update_setup();
	tm_printf("done.\n");
	tm_printf("Enabling I2S input DMA... ");
	i2s_in_dma.enable();
	tm_printf("done.\n");
	tm_printf("Registering I2S input DMA interrupt... ");
	i2s_in_dma.attachInterrupt(tm_i2s_input_isr);
	tm_printf("done.\n");
	
	/* Next, init i2s output */
	tm_printf("Allocating I2S output DMA channel... ");
	...
	tm_printf("Attaching I2S output interrupt... ");
	i2s_out_update_responsibility = update_setup();
	i2s_out_dma.attachInterrupt(tm_i2s_output_isr);
	tm_printf("done.\n");
}
...
extern "C" void i2s_in_transmit(tm_audio_block_int *block, unsigned char index)
{
	...
	/*for (tm_audio_connection *c = i2s_destination_list; c != NULL; c = c->next_dest)
	{
