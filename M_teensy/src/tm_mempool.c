...
static const char *FNAME = "tm_mempool.c";
...
float   buffer_pool[TM_BUFFER_POOL_SIZE][AUDIO_BLOCK_SAMPLES];
#endif
float  *buffer_buffer[TM_BUFFER_POOL_SIZE];

int buffer_head = 0;
int buffer_tail = TM_BUFFER_POOL_SIZE - 1;
...
FLASHMEM void init_mem_pools()
{
	...
	for (int i = 0; i < TM_BUFFER_POOL_SIZE; i++)
	{
		#ifdef BUFFER_QUEUE_STATIC
...
float *allocate_buffer()
{
	...
	if (buffer_head != buffer_tail)
	{
		...
		buffer_head = (buffer_head + 1) % TM_BUFFER_POOL_SIZE;
	}
	else
	{
...
void release_buffer(float *buffer)
{
	...
	buffer_tail = (buffer_tail + 1) % TM_BUFFER_POOL_SIZE;
...
void print_mempool_info()
{
	...
	int available_buffers = buffer_tail - buffer_head + ((buffer_tail < buffer_head) ? TM_BUFFER_POOL_SIZE : 0);
	int used_buffers = TM_BUFFER_POOL_SIZE - available_buffers;
	tm_printf("Used buffers: %d. Available buffers: %d\n", used_buffers, available_buffers);
