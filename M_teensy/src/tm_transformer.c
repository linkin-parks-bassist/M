...
static const char *FNAME = "tm_transformer.c";
...
int transformer_init_setting_array(tm_transformer *trans, int n)
{
	...
	trans->settings 	= (tm_setting**)malloc(sizeof(tm_setting*) * n);
...
void free_transformer(tm_transformer *trans)
{
	...
	tm_printf("trans->parameters = %p, trans->settings = %p, trans->data_struct = %p\n",
	...
	if (trans->settings)
	{
		tm_printf("Freeing setting array...\n");
		free(trans->settings);
	}
...
int clone_transformer(tm_transformer **dest_ptr, tm_transformer *src)
{
	...
	if (!*dest_ptr)
	{
		*dest_ptr = (tm_transformer*)malloc(sizeof(tm_transformer));
		if (!*dest_ptr)
		{
			RETURN_ERR_CODE(ERR_ALLOC_FAIL);
		}
	...
	if (src->settings)
	{
		for (int i = 0; i < dest->n_settings; i++)
		{
			if (dest->settings[i] && src->settings[i])
				memcpy(dest->settings[i], src->settings[i], sizeof(tm_setting));
		}
	}
	...
	if (src->parameters)
	{
		for (int i = 0; i < dest->n_parameters; i++)
		{
			if (dest->parameters[i] && src->parameters[i])
				memcpy(dest->parameters[i], src->parameters[i], sizeof(tm_parameter));
		}
	}
