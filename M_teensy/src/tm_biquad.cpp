...
int calc_biquad(float **dest, float **src, void *transformer_data)
{
	...
	//tm_printf("Calculating biquad transform...\n");
	tm_biquad_data *biquad = (tm_biquad_data*)transformer_data;
...
int init_biquad(tm_audio_transformer *trans, vec2i input, vec2i output, biquad_type type, float cutoff, float bandwidth, float db_gain)
{
	...
	tm_biquad_data *data_struct = (tm_biquad_data*)malloc(sizeof(tm_biquad_data));
...
{
	tm_audio_transformer *biquad = (tm_audio_transformer*)malloc(sizeof(tm_audio_transformer));
	int ret_val;
