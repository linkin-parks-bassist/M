...
int init_pipeline(m_pipeline *pipeline, int width, int height)
{
	...
	pipeline->active_node_array = (m_pipeline_node**)malloc(height * width * sizeof(m_pipeline_node*) + 2);
...
int pipeline_deactivate_nodes(m_pipeline *pipeline)
{
	if (!pipeline)
...
int pipeline_activate_node(m_pipeline *pipeline, vec2i pos)
{
	if (!pipeline)
		return ERR_PIPELINE_NULL;
...
int pipeline_deactivate_node(m_pipeline *pipeline, vec2i pos)
{
	if (!pipeline)
		return ERR_PIPELINE_NULL;
...
int compute_pipeline(m_pipeline *pipeline, int16_t *input)
{
	...
	// If the pipeline is busted, just bypass
