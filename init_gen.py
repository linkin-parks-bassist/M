...
em_fname = "./M_esp32/main/em_transformer_init.c"
em_initial = ["#include \"em.h\"", ""]
...
def tm_trans_null_ptr_guard():
	return ["	if (!trans)", "		return ERR_NULL_PTR;", ""]

def tm_big_switch(data):
	output = ["int init_transformer_default(tm_transformer *trans, uint16_t type)", "{"]
	output += tm_trans_null_ptr_guard()
...
def tm_transformer_init_function(transformer):
	...
	output = output + tm_trans_null_ptr_guard()
...
def em_init_function(transformer):
	...
	name = transformer['name']
	struct_name = enum_sanitize(name).capitalize() + 'Block'
	fn_name = f"init_{enum_sanitize(name)}"

	output.append(f"void {fn_name}(em_transformer *trans)")
	output.append("{")
	output.append(f"	r->type = {transformer_type};")
	...
	for param in transformer['parameters']:
		...
		smoothing = param.get('smoothing', 'none').upper()
		
		field = f"trans->param_{enum_sanitize(pname)}"
		output.append(f"	{field}.name = \"{pname}\";")
		output.append(f"	{field}.min = {minval}f;")
		output.append(f"	{field}.max = {maxval}f;")
		output.append(f"	{field}.default_val = {default}f;")
		output.append(f"	{field}.smoothing = SMOOTHING_{smoothing};")
