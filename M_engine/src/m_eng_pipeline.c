...
int nullify_pipeline(m_eng_pipeline *pipeline)
{
	if (!pipeline)
	{
...
int init_pipeline(m_eng_pipeline *pipeline, int width, int height)
{
	if (!pipeline)
	{
		RETURN_ERR_CODE(ERR_NULL_PTR);
	...
	m_eng_printf("Initialising pipeline...\n");
	...
	// Then do the alloc's
	//m_eng_printf("Allocating input block...\n");
	if (!(pipeline->input_node.block = allocate_block()))
		goto free_and_err_out;
	
	m_eng_printf("Allocated input node block at %p\n", pipeline->input_node.block);
	
	pipeline->input_node.active  = 1;
	
	//m_eng_printf("Allocating output block...\n");
	if (!(pipeline->output_node.block = allocate_block()))
		goto free_and_err_out;

	m_eng_printf("Allocated output node block at %p\n", pipeline->output_node.block);
	
	pipeline->output_node.active = 1;
	
	//m_eng_printf("Allocating node array...\n");
	pipeline->nodes = (m_eng_pipeline_node**)malloc(width * sizeof(m_eng_pipeline_node*));
	...
	for (int i = 0; i < width; i++)
	{
		pipeline->nodes[i] = (m_eng_pipeline_node*)malloc(height * sizeof(m_eng_pipeline_node));
			
		if (!pipeline->nodes[i])
			goto free_and_err_out;
	}
	
	//m_eng_printf("Initialising array nodes...\n");
	for (int i = 0; i < width; i++)
	{
	...
	pipeline->active_node_array = (m_eng_pipeline_node**)malloc(height * width * sizeof(m_eng_pipeline_node*) + 2);
...
free_and_err_out:
	m_eng_printf("Pipeline initialisation failed!!\n");
	free_pipeline(pipeline);
	RETURN_ERR_CODE(ERR_ALLOC_FAIL);
}

int init_bypass_pipeline(m_eng_pipeline *pipeline)
{
	m_eng_printf("Initialising bypass pipeline...\n");
	if (!pipeline)
	{
		RETURN_ERR_CODE(ERR_NULL_PTR);
	}
...
int free_pipeline(m_eng_pipeline *pipeline)
{
	// free and nullify all non-null alloc'd pointers in the pipeline struct, inc. blocks
	RETURN_ERR_CODE(ERR_UNIMPLEMENTED);
}

int pipeline_deactivate_nodes(m_eng_pipeline *pipeline)
{
	if (!pipeline)
	{
		RETURN_ERR_CODE(ERR_PIPELINE_NULL);
	}
...
int pipeline_reconfigure(m_eng_pipeline *pipeline)
{
	int ret_val = 0;
	...
	m_eng_printf("Configuring pipeline\n");
				...
				else
				{
					if (v.x == OUTPUT_NODE_X && v.y == OUTPUT_NODE_Y)
					{
						if (output_fed)
						{
							...
							goto pipeline_reconfigure_free_and_return;
						}
						else
						{
							output_fed = 1;
...
pipeline_reconfigure_alloc_fail_panic:
...
int pipeline_activate_node(m_eng_pipeline *pipeline, vec2i pos)
{
	if (!pipeline)
	{
		RETURN_ERR_CODE(ERR_PIPELINE_NULL);
	...
	{
		RETURN_ERR_CODE(ERR_POSITION_ILLEGAL);
	}
...
void regenerate_active_node_array(m_eng_pipeline *pipeline)
{
	if (!pipeline)
	{
		RETURN_VOID();
...
int pipeline_deactivate_node(m_eng_pipeline *pipeline, vec2i pos)
{
	if (!pipeline)
	{
		RETURN_ERR_CODE(ERR_PIPELINE_NULL);
	...
	{
		RETURN_ERR_CODE(ERR_NODE_PRIVATE);
	}
...
int reset_node_status(m_eng_pipeline *pipeline)
{
	if (!pipeline)
	{
		RETURN_ERR_CODE(ERR_PIPELINE_NULL);
...
m_eng_pipeline_node *pipeline_get_node(m_eng_pipeline *pipeline, vec2i pos)
{
	if (!pipeline)
	{
		RETURN_PTR(NULL);
...
int pipeline_add_transformer(m_eng_pipeline *pipeline, m_eng_transformer *trans)
{
	if (!pipeline || !trans)
	{
	...
	{
		RETURN_NEG_ERR_CODE(ERR_PIPELINE_FULL);
	}
...
int write_node(m_eng_pipeline_node *node, float *data)
{
	if (!node || !data)
	{
		RETURN_ERR_CODE(ERR_NULL_PTR);
...
int compute_pipeline(m_eng_pipeline *pipeline, int16_t *input)
{
	if (!pipeline || !input)
	{
		RETURN_ERR_CODE(ERR_NULL_PTR);
...
/*
int pipeline_add_transformer_by_type(m_eng_pipeline *pipeline, uint16_t type)
{
	if (!pipeline)
	{
		RETURN_NEG_ERR_CODE(ERR_NULL_PTR);
	...
	m_eng_transformer *trans = (m_eng_transformer*)malloc(sizeof(m_eng_transformer));
