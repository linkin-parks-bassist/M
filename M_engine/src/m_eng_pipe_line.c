...
static const char *FNAME = "m_eng_pipe_line.c";
int init_pipe_line(m_eng_pipe_line *pipeline)
{
	...
	pipeline->transformers = (m_eng_transformer**)malloc(sizeof(m_eng_transformer*) * pipeline->transformer_array_length);
...
int pipe_line_expand_transformer_array(m_eng_pipe_line *pipeline)
{
	...
	m_eng_transformer **new_ptr = NULL;
	
	do
	{
		new_ptr = (m_eng_transformer**)malloc(sizeof(m_eng_transformer*) * (pipeline->transformer_array_length + try_size));
		if (!new_ptr)
			try_size /= 2;
	...
	memcpy(new_ptr, pipeline->transformers, sizeof(m_eng_transformer*) * (pipeline->transformer_array_length));
...
int pipeline_update_transition_policy(m_eng_pipe_line *pipeline)
{
	FUNCTION_START();
...
int pipe_line_print_transformer_array(m_eng_pipe_line *pipeline)
{
	FUNCTION_START();
	...
	m_eng_printf("Printing transformer array. n_transformers = %d\n", pipeline->n_transformers);
	
	for (int i = 0; i < pipeline->n_transformers; i++)
		m_eng_printf("transformer %d: type %d, id %d\n", i, pipeline->transformers[i]->type, pipeline->transformers[i]->id);
...
int pipe_line_remove_transformer(m_eng_pipe_line *pipeline, uint16_t tid)
{
	...
	m_eng_printf("pipe_line_remove_transformer. pipeline = %p, tid = %d\n", pipeline, tid);
	if (!pipeline)
	{
		RETURN_ERR_CODE(ERR_NULL_PTR);
	}
	
	m_eng_printf("pipeline->n_transformers = %d, pipeline->transformers = %p\n", pipeline->n_transformers, pipeline->transformers);
	
	if (pipeline->transformers)
	{
		for (int j = 0; j < pipeline->n_transformers; j++)
			m_eng_printf("pipeline->transformers[%d] = %p,\n", pipeline->transformers[j]);
	}
	
	for (int i = 0; i < pipeline->n_transformers; i++)
	{
		m_eng_printf("iteration %d. pipeline->transformers[%d] = %p, pipeline->transformers[%d]->id = %d\n", i, i,
		...
		{
			m_eng_printf("this is the one! freeing it...\n");
			...
			m_eng_printf("Transformer array now: \n");
...
int pipe_line_move_transformer(m_eng_pipe_line *pipeline, uint16_t id, int new_pos)
{
	FUNCTION_START();
	...
	m_eng_printf("Moving transformer %d from position %d to position %d. The array currently:\n", id, old_pos, new_pos);
	...
	m_eng_transformer *trans = pipeline->transformers[old_pos];
	...
	m_eng_printf("And after the move:\n");
	pipe_line_print_transformer_array(pipeline);
...
int pipe_line_swap_transformers(m_eng_pipe_line *pipeline, uint16_t id1, uint16_t id2)
{
	...
	m_eng_transformer *tmp = pipeline->transformers[index1];
...
int clone_pipe_line(m_eng_pipe_line **dest_ptr, m_eng_pipe_line *src)
{
	FUNCTION_START();
	...
	m_eng_pipe_line *dest = *dest_ptr;
	
	if (!dest)
	{
		dest = (m_eng_pipe_line*)malloc(sizeof(m_eng_pipe_line));
...
int pipe_line_clone_transformer_into_position(m_eng_pipe_line *pipeline, m_eng_transformer *trans, int pos)
{
	FUNCTION_START();
