...
static const char *FNAME = "m_eng_graph.c";
...
int init_pipeline(m_eng_graph *pipeline, int width, int height)
{
	...
	m_printf("Allocated input node block at %p\n", pipeline->input_node.block);
	...
	m_printf("Allocated output node block at %p\n", pipeline->output_node.block);
...
free_and_err_out:
	m_printf("Pipeline initialisation failed!!\n");
...
{
	m_printf("Initialising bypass pipeline...\n");
	if (!pipeline)
	{
		RETURN_ERR_CODE(ERR_NULL_PTR);
	}
...
int pipeline_deactivate_nodes(m_eng_graph *pipeline)
{
	if (!pipeline)
	{
...
int pipeline_activate_node(m_eng_graph *pipeline, vec2i pos)
{
	if (!pipeline)
	{
		RETURN_ERR_CODE(ERR_PIPELINE_NULL);
...
void regenerate_active_node_array(m_eng_graph *pipeline)
{
	if (!pipeline)
	{
		RETURN_VOID();
...
int pipeline_deactivate_node(m_eng_graph *pipeline, vec2i pos)
{
	if (!pipeline)
	{
		RETURN_ERR_CODE(ERR_PIPELINE_NULL);
...
int reset_node_status(m_eng_graph *pipeline)
{
	if (!pipeline)
	{
		RETURN_ERR_CODE(ERR_PIPELINE_NULL);
...
int compute_pipeline(m_eng_graph *pipeline, int16_t *input)
{
	...
	if (pipeline->err_flags & PIPELINE_ERR_FLAG_NODE_CONFLICT
	 ...
	 || pipeline->err_flags & PIPELINE_ERR_FLAG_UNCONFIGURED)
	{
		//m_printf("pipeline is bungled; bypass\n", pipeline->output_node.block->data);
