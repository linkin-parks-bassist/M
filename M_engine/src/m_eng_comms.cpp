...
int et_msg_sanity_check(et_msg msg, int len)
{
	...
	m_eng_printf("Message sanity check. msg.type = %d. length = %d. expected length = %d.\n", msg.type, len, et_message_data_length(msg));
...
void handle_esp32_message(et_msg msg)
{
	...
	switch (msg.type)
	{
		...
		case ET_MESSAGE_CREATE_PROFILE:
			m_eng_printf("Creating new profile...\n");
			ret_val = m_eng_context_new_profile(&global_cxt);
			
			m_eng_printf("ret_val: %d\n", ret_val);
		...
		case ET_MESSAGE_APPEND_TRANSFORMER:
			m_eng_printf("Creating new transformer of type %d to profile %d...\n", arg16_2, arg16_1);
			
			ret_val = cxt_append_transformer_to_profile(&global_cxt, arg16_1, arg16_2);
			
			if (ret_val < 0)
			{
				m_eng_printf("Appending transformer failed with error %s\n", m_error_code_to_string(-ret_val));
				response = create_te_msg_error(-ret_val);
			}
			else
			{
				m_eng_printf("Success. Obtained transformer id %d; sending back\n", ret_val);
				response = create_te_msg_transformer_id(arg16_1, ret_val);
				m_eng_printf("Response created\n");
			}
			break;
		
		case ET_MESSAGE_MOVE_TRANSFORMER:
			m_eng_printf("Moving transformer...\n");
		...
		case ET_MESSAGE_REMOVE_TRANSFORMER:
			m_eng_printf("Removing transformer...\n");
		...
		case ET_MESSAGE_GET_TRANSFORMER_ID:
			m_eng_printf("esp32 asks: what is the id of the transformer in position %d in profile %d? answer: %d\n", arg16_2, arg16_1, cxt_get_tid_by_pos(&global_cxt, arg16_1, arg16_2));
		...
		case ET_MESSAGE_SET_PARAM_VALUE:
			...
			m_eng_printf("Request to set parameter %d.%d.%d to value %f\n", arg16_1, arg16_2, arg16_3, value_f);
		...
		case ET_MESSAGE_GET_SETTING_VALUE:
			...
			{
				m_eng_printf("Requested setting doesn't exist !\n");
				response.type = TE_MESSAGE_BAD_REQUEST;
			}
			break;
		...
		case ET_MESSAGE_SWITCH_PROFILE:
			m_eng_printf("Profile switch requested; to profile %d\n", arg16_1);
			if (arg16_1 < global_cxt.n_profiles)
			{
				...
				m_eng_printf("Request valid; swwitching. Error code: %d\n", ret_val);
			}
			else
			{
				m_eng_printf("No such profile exists!\n");
...
void i2c_receive_isr(int n)
{
	...
	//for (int i = 0; i < n; i++)
	//	m_eng_printf("%d%s", receive_buffer[i], (i < n - 1) ? ", " : "\n");
...
void i2c_request_isr()
{
	...
	if (response_ready)
	{
		...
		for (int i = 0; i < TE_MESSAGE_MAX_TRANSFER_LEN; i++)
			m_eng_printf("0x%02x%s", response_buffer[i], (i == TE_MESSAGE_MAX_TRANSFER_LEN - 1) ? "\n" : " ");
		#endif
	}
	else
	{
