...
int init_profile(m_eng_profile *profile)
{
	...
	profile->front_pipeline = (m_eng_pipe_line*)malloc(sizeof(m_eng_pipe_line));
	...
	profile->back_pipeline = (m_eng_pipe_line*)malloc(sizeof(m_eng_pipe_line));
...
int profile_print_job_list(m_eng_profile *profile)
{
	...
	m_eng_printf("Printing job list.\n");
	
	int i = 0;
	m_eng_pipe_line_mod_linked_list *current = profile->jobs;
	
	while (current)
	{
		m_eng_printf("Job %i: ", i);
		switch (current->data.type)
		{
			case PIPE_LINE_MOD_APPEND_TRANSFORMER:
				m_eng_printf("append transformer of type %d\n", current->data.data);
				break;
			
			
			case PIPE_LINE_MOD_MOVE_TRANSFORMER:
				m_eng_printf("move transformer %d to position %d\n", current->data.tid, current->data.data);
				break;
...
int profile_print_ujob_list(m_eng_profile *profile)
{
	...
	m_eng_printf("Printing ujob list.\n");
	
	int i = 0;
	m_eng_pipe_line_mod_linked_list *current = profile->ujobs;
...
int profile_apply_pipeline_mod(m_eng_profile *profile, m_eng_pipe_line_mod mod)
{
	...
	m_eng_printf("Applying a type %d pipeline mod\n", mod.type);
	if (!profile)
	{
		RETURN_ERR_CODE(ERR_NULL_PTR);
	}
	
	m_eng_pipe_line_mod_linked_list *nl;
	...
	{
		m_eng_printf("There is no pipeline swap occuring; safe to apply to back pipeline\n");
		ret_val = apply_pipe_line_mod(profile->back_pipeline, mod);
		
		m_eng_printf("Applied\n");
		...
		nl = m_eng_pipe_line_mod_linked_list_append(profile->jobs, mod);
		if (nl)
		{
			profile->jobs = nl;
		}
	...
	{
		m_eng_printf("There is a pipeline swap occuring. I will log the job for later.\n");
		
		nl = m_eng_pipe_line_mod_linked_list_append(profile->ujobs, mod);
		if (nl)
		{
			profile->ujobs = nl;
		}
	...
	m_eng_printf("Function returned\n");
	RETURN_ERR_CODE(ret_val);
}
...
int profile_update(m_eng_profile *profile)
{
	...
	m_eng_pipe_line_mod_linked_list *next;
...
int profile_process(m_eng_profile *profile, float *dest, float *src)
{
	...
	{	
		...
		if (!profile->back_pipeline)
		{
			//m_eng_printf("ERROR: Back pipeline doesn't exist!!\n");
			RETURN_ERR_CODE(ERR_BAD_ARGS);
		}
		...
		/*if (!profile->back_pipeline_warmed_up)
		{
			...
			{
				m_eng_printf("Could allocate temporary buffer to warm up back pipeline!\n");
			}
		}*/
		...
		if (profile->pipeline_swap_type == TRANSITION_TAIL)
		{
			...
			{
				m_eng_printf("ERROR: compute_pipe_line ERROR CODES (%d, %d)\n", ret_val, ret_val_2);
			}
		...
		if (swap_complete)
		{
			m_eng_pipe_line *tmp = profile->front_pipeline;
	...
	{
		if (local_dest)
			ret_val = compute_pipe_line(profile->front_pipeline, local_dest, src);
		else
			ret_val = compute_pipe_line(profile->front_pipeline, local_dest, src);
		
		if (ret_val != NO_ERROR)
		{
			m_eng_printf("PIPELINE COMPUTATION ERROR %d\n", ret_val);
		}
	}
...
int profile_trigger_pipeline_swap(m_eng_profile *profile)
{
	...
	m_eng_printf("Initiating a pipeline swap\n");
	if (!profile)
	{
		RETURN_ERR_CODE(ERR_NULL_PTR);
	}
...
int profile_scheduled_maintainance(m_eng_profile *profile)
{
	...
	{
		m_eng_printf("Warning: inconsistent back pipeline detected in profile %p. Regenerating...\n", profile);
