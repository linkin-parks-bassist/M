...
typedef struct
{
	...
	m_eng_pipeline_node input_node, output_node;
	
	m_eng_pipeline_node **nodes;
	m_eng_pipeline_node **active_node_array;
	m_eng_transformer *transformers[MAX_PIPELINE_TRANSFORMERS];
...
int pipeline_activate_node(m_eng_pipeline *pipeline, vec2i);
...
int reset_nodes(m_eng_pipeline *pipeline);
